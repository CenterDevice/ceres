...
pub fn get_story(client: &ReqwestClient, project_id: u64, story_id: u64, token: &str) -> impl Future<Item = Story, Error = Error> {
   let url = format!(
...
pub fn get_project_members(client: &ReqwestClient, project_id: u64, token: &str) -> impl Future<Item = Vec<ProjectMember>, Error = Error> {
   let url = format!(
